"""Poprawka tytulow: Raj i grzech pierworodny (Peter Paul Rubens)."""
...
NEW_PL_TITLE = "Raj i grzech pierworodny (lub Adam i Ewa w raju)"
ORIGINAL_TITLE = (
    "Het aardse paradijs met de zondeval van Adam en Eva (of De zondeval van Adam en Eva)"
)
ENGLISH_TITLE = "The Garden of Eden with the Fall of Man"
LOCALE_TITLES: dict[str, str] = {
    "en": ENGLISH_TITLE,
    "de": (
        "Das irdische Paradies zeigt den Sündenfall von Adam und Eva (oder Der Sündenfall)"
    ),
    "fr": "Le Paradis terrestre avec la chute d'Adam et Ève",
    "es": "El paraíso terrenal con la caída de Adán y Eva",
    "nl": ORIGINAL_TITLE,
    "it": "Il paradiso terrestre con la caduta di Adamo ed Eva",
}
OLD_LOCALE_TITLES: dict[str, tuple[str, ...]] = {
    ...
    "fr": (
        ...
        "Le Paradis terrestre avec la chute d'Adam et Ève",
    ),
    "es": (
        "El Paraíso terrenal con la caída del hombre",
        "El paraíso terrenal con la caída de Adán y Eva",
    ),
    "nl": ("Het aardse paradijs met de zondeval van Adam en Eva",),
    "it": (
        "Il Paradiso terrestre con il peccato originale",
        "Il paradiso terrestre con la caduta di Adamo ed Eva",
    ),
}
...
def main() -> int:
    ...
    for old in OLD_PL_TITLES:
        if old in pl_body and old != NEW_PL_TITLE:
    ...
    de2 = get_translated_fields(shop, token, gid, "de").get("body_html") or ""
    print("\nWeryfikacja:")
    ...
    print("  DE tytul:", extract_display_title_from_body_html(de2))
    print(
