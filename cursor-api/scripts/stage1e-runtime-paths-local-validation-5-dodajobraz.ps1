...
$validationRoot = Join-Path $env:TEMP ("gicleeapp-stage1e-5-dodajobraz-validation-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    try {
        Write-Host ""
        Write-Host "=== Stage 1E.1-1E.5 DodajObraz runtime path tests ==="
        python -m pytest `
            ...
            -q
        if ($LASTEXITCODE -ne 0) { throw "Stage 1E.5 DodajObraz tests failed with exit code $LASTEXITCODE." }
        Write-Host ""
        Write-Host "=== Compile Stage 1E.5 DodajObraz modules ==="
        python -m compileall -q `
            Komponenty/dodajobraz/description_update.py `
            Komponenty/dodajobraz/templates.py `
            Komponenty/dodajobraz/product_template_assignments.py `
            Komponenty/dodajobraz/r2_usage.py `
            Komponenty/dodajobraz/markets.py `
            Komponenty/dodajobraz/market_variant_prices.py
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "STAGE 1E.5 DODAJOBRAZ LOCAL VALIDATION COMPLETE"
