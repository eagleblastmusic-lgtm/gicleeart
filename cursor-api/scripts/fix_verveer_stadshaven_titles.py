"""Poprawka tytulow: Port miejski latem (Salomon Verveer)."""
...
OLD_PL_TITLES = (
    "Miejskie nabrzeże latem",
    "Miejska przystań latem (lub Port miejski latem)",
    "Miejska przystań latem",
    "Port miejski latem",
)
NEW_PL_TITLE = format_title_alternative_parenthetical(
    "Port miejski latem (lub Miejska przystań latem)",
    "pl",
)
ORIGINAL_TITLE = "Stadshaven in de zomer"
ENGLISH_TITLE = "Town dock in summer"
LOCALE_TITLES: dict[str, str] = {
    "en": ENGLISH_TITLE,
    "de": "Stadthafen im Sommer",
    "fr": "Port de la ville en été",
    "es": "Puerto de la ciudad en verano",
    "nl": ORIGINAL_TITLE,
    "it": "Porto cittadino in estate",
}
...
def _print_changes(before: dict, after: dict) -> None:
    ...
    for key in ("product", "pl", *SUPPORTED_LANGS):
        ...
        if key == "product":
            ...
            label = key.upper() if len(key) == 2 else key
            for field in ("display", "original"):
                bk = f"{label} {field}" if key != "pl" else f"PL {field}"
                if key == "pl":
                    bk = f"PL {field}"
                elif len(key) == 2:
                    bk = f"{key.upper()} {field}"
                if b.get(field) != a.get(field):
                    print(f"  {bk}: {b.get(field)!r} -> {a.get(field)!r}")
                    changed = True
    if not changed:
        print("  (brak zmian w tytulach)")
