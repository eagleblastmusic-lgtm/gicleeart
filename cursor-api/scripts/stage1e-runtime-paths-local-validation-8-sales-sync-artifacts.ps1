...
$validationRoot = Join-Path $env:TEMP ("gicleeapp-stage1e-8-sales-sync-artifacts-validation-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    try {
        Write-Host ""
        Write-Host "=== Stage 1E.1-1E.8 sales sync artifact runtime path tests ==="
        python -m pytest `
            ...
            -q
        if ($LASTEXITCODE -ne 0) { throw "Stage 1E.8 sales sync artifact tests failed with exit code $LASTEXITCODE." }

        Write-Host ""
        Write-Host "=== Compile Stage 1E.8 sales sync artifact modules ==="
        python -m compileall -q `
            Komponenty/dokumentysprzedazy/orders_sync.py `
            Komponenty/dokumentysprzedazy/export_monthly.py
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "STAGE 1E.8 SALES SYNC ARTIFACTS LOCAL VALIDATION COMPLETE"
