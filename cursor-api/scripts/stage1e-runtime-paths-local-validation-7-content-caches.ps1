...
$validationRoot = Join-Path $env:TEMP ("gicleeapp-stage1e-7-content-caches-validation-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    try {
        Write-Host ""
        Write-Host "=== Stage 1E.1-1E.7 content cache runtime path tests ==="
        python -m pytest `
            ...
            -q
        if ($LASTEXITCODE -ne 0) { throw "Stage 1E.7 content cache tests failed with exit code $LASTEXITCODE." }
        Write-Host ""
        Write-Host "=== Compile Stage 1E.7 content cache modules ==="
        python -m compileall -q `
            Komponenty/karuzela/quotes_service.py `
            Komponenty/tldobio/service.py
    ...
    Write-Host ""
    Write-Host "============================================================"
    Write-Host "STAGE 1E.7 CONTENT CACHES LOCAL VALIDATION COMPLETE"
