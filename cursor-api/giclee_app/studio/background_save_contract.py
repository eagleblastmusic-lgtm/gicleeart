"""Save contract + dry-run tła section_background — Studio Preview (F5.4a).
...
def build_background_save_dry_run(
    ...
    change_summary = _build_change_summary(current, target_pl)
...
def format_dry_run_summary(dry_run: BackgroundSaveDryRun) -> str:
    ...
    if not dry_run.writable:
        lines.append(
            "Uwaga: draft nie zawiera pliku assetu — realny zapis wymaga ref lub edytora inline."
        )
    if dry_run.fields_touched:
        fields = ", ".join(dry_run.fields_touched)
        lines.append(f"Pola (F5.4b): {fields}")
    lines.append("")
    lines.append(F54A_DISCLAIMER)
    return "\n".join(lines)
...
def _build_change_summary(current: SectionBgStatus, target_pl: str) -> str:
...
def _is_writable(current: SectionBgStatus, target_kind: SectionSaveKind) -> bool:
    """F5.4b zapis bez ref w draft — dozwolony tylko przy zgodności typu i istniejącym tle."""
