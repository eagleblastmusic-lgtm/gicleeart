...
def _build_ui(host: tk.Tk) -> None:
    ...
    def _add_image_row(parent: ttk.Frame, fld: HomeField, zone: HomeZone, initial: str) -> None:
        ...
        var = tk.StringVar(
            value=shopify_ref_label(initial) if fld.kind == "shopify_image" else (initial or "(brak)")
        )
        ...
        def _refresh_thumb(ref: str = "") -> None:
            if fld.kind == "theme_asset":
                ...
                _set_thumbnail(thumb, shopify_ref=full if full.startswith("shopify://") else "")

        def _upload_path(p: Path) -> None:
            if p.suffix.lower() not in _IMAGE_SUFFIXES:
                messagebox.showerror(APP_TITLE, "Dozwolone: JPG, PNG, WebP.", parent=host)
                return
        ...
        ttk.Button(meta, text="Wgraj grafikę…", command=_pick).pack(anchor="w", pady=(4, 0))
        if _HAS_DND:
    ...
    def _show_zone(zone: HomeZone) -> None:
        ...
        for fld in zone.fields:
            ...
            if fld.kind in ("shopify_image", "theme_asset"):
    ...
    def _reload(*, keep_zone: bool = True) -> None:
        try:
            ...
            status_var.set("Błąd wczytywania plików motywu.")
            messagebox.showerror(APP_TITLE, str(exc), parent=host)
            return
        ...
        if missing:
            status_var.set(f"Uwaga: {len(missing)} pól nie znaleziono w index.json.{scan_note}")
        else:
            status_var.set(f"Wczytano {len(HOME_ZONES)} sekcji.{scan_note}")
        _refresh_zone_list()
        ...
        *,
        ...
        def _toggle_diff() -> None:
            if show_diff_var.get():
                ...
                backup_path = backups[0]["index_path"] if backups else index_template_path().with_suffix(".json.bak")
                if not backup_path.is_file():
                    backup_path = index_template_path().with_name("index.json.bak")
    ...
    def _save_all() -> None:
        ...
        status_var.set(f"Zapisano. Kopia: …/data/backups/{backup_note}")
        show_toast(host, "Zapisano index.json, ustawienia i assets motywu.")
    ...
    def _open_theme_dev_preview() -> None:
        if theme_dev_port_open():
            webbrowser.open(preview_url(local=True))
        ...
        def poll_ready(attempt: int = 0) -> None:
            ...
            if attempt >= 90:
                append("—" * 40)
                append("Timeout — serwer nie odpowiada na porcie 9292. Sprawdź log Shopify CLI.")
        ...
        def worker() -> None:
            try:
                ...
                home_features_mod._theme_dev_proc = proc
                ...
                def reader() -> None:
                    while proc.poll() is None:
                        ...
                        if line:
                            text = line.rstrip()
                            host.after(0, lambda t=text: append(t))
    ...
    ttk.Button(bottom, text="Odśwież z pliku", command=lambda: _reload()).pack(side="left")
