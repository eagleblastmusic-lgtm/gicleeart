...
# Zgodne z «Pliki startowe dla GPT/GICLEE_CURSOR_ARCHITECT_CLEAN_PACK_v35.md».
CLEAN_PACK_V35_ACTIVE_FILES: tuple[str, ...] = (
    ...
    "README_GICLEE_CURSOR_ARCHITECT_UPDATE_v35.md",
    "GICLEE_CURSOR_ARCHITECT_CLEAN_PACK_v35.md",
...
def list_starter_markdown_files(folder: Path | None = None) -> list[Path]:
    """Pliki .md z manifestu CLEAN_PACK v35 (bez archiwalnych wersji na dysku)."""
    ...
    files: list[Path] = []
    missing: list[str] = []
    for name in CLEAN_PACK_V35_ACTIVE_FILES:
        path = root / name
        if path.is_file():
            files.append(path)
        else:
    ...
    if missing:
        raise FileNotFoundError(
            f"Brak plików CLEAN_PACK v35 w {root}: {', '.join(missing)}"
        )
    return files


def build_starter_knowledge_zip(folder: Path | None = None) -> Path:
    """Tworzy giclee_cursor_architect_knowledge.zip wg manifestu CLEAN_PACK v35."""
    root = (folder or gpt_starter_files_dir()).resolve()
...
def read_compact_instructions(folder: Path | None = None) -> str:
    """Treść GICLEE_CURSOR_ARCHITECT_INSTRUCTIONS_COMPACT_v35.md (główne instrukcje w ZIP-ie)."""
    root = (folder or gpt_starter_files_dir()).resolve()
    path = root / "GICLEE_CURSOR_ARCHITECT_INSTRUCTIONS_COMPACT_v35.md"
    if not path.is_file():
