...
(function (root, factory) {
  ...
  function shopifySizeToFormatName(shopifySize) {
    var key = String(shopifySize || "").toUpperCase();
  ...
  function verdictForPpi(ppi) {
    if (!Number.isFinite(ppi)) {
      return {
        label: "Brak danych",
        note: "Wgraj obraz, aby zobaczyć analizę.",
        tone: "gray",
      };
    }
    if (ppi >= VERDICT_THRESHOLD_EXCELLENT) {
      return {
        label: "Najwyższa jakość",
        note: "Maksymalna ostrość, Fine Art, Wydruk premium. Najlepszy odbiór z 20–30 cm",
        tone: "blue",
      };
    }
    if (ppi >= VERDICT_THRESHOLD_VERY_GOOD) {
      return {
        label: "Bardzo dobra jakość",
        note: "Praktycznie brak różnicy względem 300 PPI przy normalnym odbiorze z 30–50 cm",
        tone: "green",
      };
    }
    if (ppi >= VERDICT_THRESHOLD_GOOD) {
      return {
        label: "Dobra jakość",
        note: "Nadal dobry wydruk, ale widać mniejszy zapas detalu, zalecana odległość oglądania to 50–80 cm",
        tone: "green-soft",
      };
    }
    if (ppi >= VERDICT_THRESHOLD_FAIR) {
      return {
        label: "Przeciętna jakość",
        note: "Efekt wystawowy, lekko miększy, akceptowalne z odległości 80–120 cm",
        tone: "amber",
      };
    }
    if (ppi >= VERDICT_THRESHOLD_MIN) {
      return {
        label: "Niezalecana",
        note: "To już jest za mało PPI do oglądania z bliska, jedynie z dystansu 120 cm i więcej wygląda dobrze",
        tone: "red",
      };
    }
    return {
      label: "Beznadziejna",
      note: "Rozdzielczość krytycznie niska — wyraźna pixelacja i brak detalu nawet z daleka; do tego formatu nie nadaje się",
      tone: "red",
    };
  }
